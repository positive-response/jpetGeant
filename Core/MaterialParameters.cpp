...
const MaterialConstants MaterialParameters::XAD4 = MaterialConstants( { 2.5 * ns, 10 * ns, 90 * ns }, { 3.3, 2.8, 44.8 }, 0.125 * ns, 1.0/3.0, { 0.45 * ns }, { 100 } );
...
void MaterialParameters::Set_ComponentsIntensities()
{
  ...
  foPs3GIntensities.clear();
  fdirectIntensities.clear();
  G4double oPsTotalProbability = 0.;
  ...
  fpPsIntensity = oPsTotalProbability*fpPsFraction/100; // Fraction from 0 to 1;
  ...
  for( unsigned j=0; j<fdirectProbabilities.size(); j++ )
  {
    temp = (100 - oPsTotalProbability - 100*fpPsIntensity)*fdirectProbabilities[j]/(100*directTotalProbability); // It should work hmmm... need to check this
    if( temp > 0. && temp <= 1. )
      fdirectIntensities.push_back( temp/1 );
    else
